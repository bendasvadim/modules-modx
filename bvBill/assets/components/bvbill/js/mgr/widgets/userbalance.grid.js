bvBill.grid.UserBalance = function (config) {
    config = config || {};
    if (!config.id) {
        config.id = 'bvbill-grid-userbalance';
    }
    Ext.applyIf(config, {
    ...
    bvBill.grid.UserBalance.superclass.constructor.call(this, config);
...
Ext.extend(bvBill.grid.UserBalance, MODx.grid.Grid, {
    windows: {},
        ...
        MODx.Ajax.request({
            ...
            listeners: {
                success: {
                    fn: function (r) {
                        var w = MODx.load({
                            xtype: 'bvbill-userbalance-window-update',
                            id: Ext.id(),
                            record: r,
                            listeners: {
                                success: {
    ...
    getColumns: function () {
        return [{
            ...
            header: _('bvbill_userbalance_username'),
            dataIndex: 'username',
            sortable: true,
            width: 150
        }, {
            header: _('bvbill_userbalance_balance'),
            dataIndex: 'balance',
            sortable: true,
            width: 100
        }, {
...
Ext.reg('bvbill-grid-userbalance', bvBill.grid.UserBalance);
