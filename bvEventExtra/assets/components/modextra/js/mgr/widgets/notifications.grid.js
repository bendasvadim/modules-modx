modExtra.grid.Notifications = function (config) {
    config = config || {};
    if (!config.id) {
        config.id = 'modextra-grid-notifications';
    }
    Ext.applyIf(config, {
    ...
    modExtra.grid.Notifications.superclass.constructor.call(this, config);
...
Ext.extend(modExtra.grid.Notifications, MODx.grid.Grid, {
    windows: {},
    ...
    createNotification: function (btn, e) {
        var w = MODx.load({
            xtype: 'modextra-notification-window-create',
            id: Ext.id(),
            listeners: {
                success: {
                    fn: function () {
        ...
        MODx.Ajax.request({
            ...
            listeners: {
                success: {
                    fn: function (r) {
                        var w = MODx.load({
                            xtype: 'modextra-notification-window-update',
                            id: Ext.id(),
                            record: r,
                            listeners: {
                                success: {
    ...
    removeNotification: function () {
        ...
        MODx.msg.confirm({
            title: ids.length > 1
                ? _('modextra_notification_remove')
                : _('modextra_notification_remove'),
            text: ids.length > 1
                ? _('modextra_notification_remove_confirm')
                : _('modextra_notification_remove_confirm'),
            url: this.config.url,
            params: {
    ...
    getColumns: function () {
        return [{
            ...
            dataIndex: 'createdon',
            renderer: modExtra.utils.formatDate,
            sortable: true,
            width: 150
        }, {
    ...
    getTopBar: function () {
        return [{
            text: '<i class="icon icon-plus"></i>&nbsp;' + _('modextra_notification_create'),
...
Ext.reg('modextra-grid-notifications', modExtra.grid.Notifications);
